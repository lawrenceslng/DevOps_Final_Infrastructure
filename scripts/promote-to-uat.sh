...
promote_microservice() {
  ...
  # Check if already an rc-1 version
  if [[ "$VERSION" == *-rc-1 ]]; then
    echo "⚠️ $REPO already has -rc-1, skipping..."
    return
  fi
...
#   echo "New VERSION for $REPO: $NEW_VERSION"
  ...
  curl -X PUT -H "Authorization: Bearer $GITHUB_TOKEN" \
    ...
    -d @- <<EOF
{
  "message": "Bump version after QA approval: $NEW_VERSION",
...
echo "🎉 All requested microservices have been bumped and promoted to RC!"
